...
def execute_saga(saga):
    ...
    # Caso de error de saga
        ...
        # Si ocurre un error, se manejan las compensaciones automáticamente
        return jsonify({
            "error": str(e.action),
...
def handle_response(add_purchase_url, purchase_data):

    try:
        response = requests.post(add_purchase_url, json=purchase_data)
